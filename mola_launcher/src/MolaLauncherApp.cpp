...
void MolaLauncherApp::shutdown()
{
  ...
  MRPT_LOG_INFO_STREAM(
      "shutdown(): Shutting down " << running_threads_.size() << " module threads...");
  ...
  for (auto& ds : running_threads_)
  {
    if (ds.second.executor.joinable())
    {
      ...
      MRPT_LOG_DEBUG_FMT("shutdown(): stopping '%s'.", ds.second.name.c_str());
...
void MolaLauncherApp::setup(
    const mrpt::containers::yaml& cfg, const std::optional<std::string>& basePath)
{
  MRPT_TRY_START
  ...
  if (profiler_.isEnabledKeepWholeHistory()) profiler_dtor_save_stats_.emplace(profiler_);
  ...
  for (const auto& section : lstSections)
  {
    ...
    ASSERTMSG_(cfg.has(sectName), "Missing YAML required entry: `" + sectName + "`");
    ...
    for (const auto& dsMap : cfg_blk.asSequence())
    {
      ...
      ASSERT_(paramsBlock.isMap() || paramsBlock.isScalar() || paramsBlock.isNullNode());
      if (paramsBlock.isScalar() && !paramsBlock.isNullNode())
      {
        const auto paramsFile = paramsBlock.as<std::string>();

        const auto absPathParamsFile = relative_to_abs_path(paramsFile, basePath);
        ...
        for (const auto& [k, v] : old.asMap()) info.yaml_cfg_block[k.as<std::string>()] = v;
      }

      MRPT_LOG_INFO_STREAM(
          "Instantiating module `" << ds_label << "` of type `" << ds_classname << "`");
      // Create object (needs to be registered):
      info.impl = sectGenerator(ds_classname);
      ASSERTMSG_(
          info.impl, mrpt::format(
      ...
      const auto verbLvl = ds.getOrDefault<std::string>("verbosity_level", "");
      if (!verbLvl.empty())
      {
        verb_level = mrpt::typemeta::TEnumType<mrpt::system::VerbosityLevel>::name2value(verbLvl);
      }
      ...
      info.execution_rate  = ds.getOrDefault<double>("execution_rate", 1.0);
      ...
      if (profiler_.isEnabledKeepWholeHistory())
      {
        ...
        info.impl->profiler_dtor_save_stats_.emplace(info.impl->profiler_);
      }

      info.impl->nameServer_ =
          std::bind(&MolaLauncherApp::nameServerImpl, this, std::placeholders::_1);
    }
  }
...
void MolaLauncherApp::spin()
{
  struct RunningGuard
  {
    RunningGuard(const std::function<void()>& runAtStart, const std::function<void()>& runAtEnd)
        : runAtEnd_(runAtEnd)
    {
      runAtStart();
    }
  ...
  RunningGuard runningGuard(
      [this]() { spin_is_running_ = true; }, [this]() { spin_is_running_ = false; });
  ...
  // Sort by launch priority:
  {
    ...
    std::transform(
        running_threads_.begin(), running_threads_.end(), std::inserter(lst, lst.begin()),
        [](auto& ds) { return std::make_pair(ds.second.launch_priority, ds.second.name); });
    ...
    for (auto& name : lst)
    {
      ...
      ds.executor = std::thread(&MolaLauncherApp::executor_thread, this, std::ref(ds));
      ...
      if (launcher_params_.enforce_initialize_one_at_a_time)
      {
        ...
        thread_launch_condition_.wait_for(
            ...
            [&ds, this] { return !threads_must_end_ && ds.initialization_done; });
      }
    }
  }
...
void MolaLauncherApp::executor_thread(InfoPerRunningThread& rds)
{
  try
  {
    ...
    // Initilize:
    MRPT_LOG_DEBUG_STREAM(
        "Thread started for module named `" << rds.name
                                            << "` (launch priority=" << rds.launch_priority << ")");
    ...
    if (launcher_params_.enforce_initialize_one_at_a_time)
      lock = std::make_unique<std::unique_lock<std::mutex>>(thread_launch_init_mtx_);
    ...
    if (rds.impl->requestedShutdown())
    {
      ...
      MRPT_LOG_INFO_STREAM("Shutdown requested by module `" << rds.name << "`");
    }
  }
  catch (const std::exception& e)
  {
    MRPT_LOG_ERROR_STREAM(
        "Error: Will shutdown since thread for module named `" << rds.name
                                                               << "` ended due to an exception:\n"
    ...
    MolaLauncherApp::scanForModuleSharedDirectories() const
{
  ...
  {
    MRPT_LOG_DEBUG_FMT("[scanForModuleSharedDirectories]: Searching under: `%s`", path.c_str());
...
std::string MolaLauncherApp::findModuleSharedDir(const std::string& moduleName) const
{
  std::map<module_name_t, module_shared_path_t> lst = scanForModuleSharedDirectories();
