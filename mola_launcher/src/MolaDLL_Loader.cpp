...
const std::map<std::string, LoadedModules>& get_loaded_modules() { return loaded_lib_handled; }
...
#define DLL_EXT "dll"
  ...
  {
    app.logStr(
        LVL_DEBUG, mrpt::format("[load modules]: Searching for modules under: `%s`", path.c_str()));
    ...
    for (const auto& lib : lst)
    {
      ...
      // Check if already loaded?
      {
        ...
        if (handle != nullptr)
        {
          app.logStr(LVL_DEBUG, mrpt::format("Skipping already loaded lib: %s", lib.name.c_str()));
          continue;  // skip
        }
      }
      ...
      app.logStr(
          LVL_DEBUG, mrpt::format("[load modules]: Successfully loaded: `%s`", lib.name.c_str()));
