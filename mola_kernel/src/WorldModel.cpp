...
// arguments: class_name, parent_class, class namespace
IMPLEMENTS_SERIALIZABLE(WorldModelData, mrpt::serialization::CSerializable, mola)
...
WorldModel::WorldModel()
{
  ...
  data_.map_name_ = "mola_map_" + mrpt::system::fileNameStripInvalidChars(
  ...
  MRPT_LOG_INFO_STREAM("=== Setting map storage base directory: `" << map_base_dir_ << "` ===");

  if (!mrpt::system::createDirectory(map_base_dir_))
    THROW_EXCEPTION_FMT("Error creating directory: `%s`", map_base_dir_.c_str());
...
struct ContainerDeque : public BASE
{
  ...
  std::pair<ID, T*> emplace_back(T&& e) override
  {
    ...
    std::visit(
        overloaded{
            [new_id](Tbase& b) { b.my_id_ = new_id; }, [new_id](Tother& o) { o->my_id_ = new_id; },
...
template <class T, class BASE, class Tbase, class Tother, typename ID, const char* Tstr>
struct ContainerFastMap : public BASE
{
  ...
  const T& by_id(const ID id) const override
  {
    ...
    {
      THROW_EXCEPTION(
          mrpt::format("`%s` not found with id=`%lu`", Tstr, static_cast<unsigned long>(id)));
    }
    return it->second;
  }
  T& by_id(const ID id) override
...
using EntitiesContainerDeque =
    ContainerDeque<Entity, WorldModelData::EntitiesContainer, EntityBase, EntityOther, id_t>;
using FactorsContainerDeque =
    ContainerDeque<Factor, WorldModelData::FactorsContainer, FactorBase, FactorOther, fid_t>;
...
using EntitiesContainerFastMap = ContainerFastMap<
    Entity, WorldModelData::EntitiesContainer, EntityBase, EntityOther, id_t, e_str>;

using FactorsContainerFastMap = ContainerFastMap<
    Factor, WorldModelData::FactorsContainer, FactorBase, FactorOther, fid_t, f_str>;
...
const Factor& WorldModel::factor_by_id(const fid_t id) const { return data_.factors_->by_id(id); }
Factor&       WorldModel::factor_by_id(const fid_t id) { return data_.factors_->by_id(id); }

std::vector<mola::id_t> WorldModel::entity_all_ids() const { return data_.entities_->all_ids(); }

std::vector<mola::fid_t> WorldModel::factor_all_ids() const { return data_.factors_->all_ids(); }
...
std::set<mola::id_t> WorldModel::entity_neighbors(const mola::id_t id) const
{
  ...
  ASSERTMSG_(it_ns != data_.entity_connected_factors_.end(), "Unknown entity `id`");
...
const annotations_data_t& WorldModel::entity_annotations_by_id(const id_t id) const
{
  MRPT_START
...
std::vector<mola::id_t> WorldModel::findEntitiesToSwapOff()
{
  ...
  for (auto it_ent = data_.entity_last_access_.begin(); it_ent != data_.entity_last_access_.end();)
  {
...
void WorldModel::spinOnce()
{
  ...
  if (!aged_ids.empty())
  {
    ...
    for (auto id : aged_ids) mola::entity_get_base(data_.entities_->by_id(id)).unload();

    entities_unlock_for_write();

    MRPT_LOG_DEBUG_STREAM("Swapped-off to disk: " << aged_ids.size() << " map entities.");
  }
...
void WorldModelData::serializeFrom(mrpt::serialization::CArchive& in, uint8_t version)
{
  // Ensure lock:
  {
    auto el = mrpt::lockHelper(entities_mtx_);
    ...
    case 0:
    {
      ...
      for (auto eid : entity_ids)
      {
        ...
        Entity e = in.ReadVariant<
            std::monostate, RefPose3, RelPose3, RelPose3KF, RelDynPose3KF, LandmarkPoint3,
            EntityOther>();
