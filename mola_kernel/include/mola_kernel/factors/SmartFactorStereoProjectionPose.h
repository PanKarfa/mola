...
class SmartFactorStereoProjectionPose : public FactorBase
{
 ...
 public:
  ...
  SmartFactorStereoProjectionPose(
      double sigma_xleft, double sigma_xright, double sigma_y, BackEndBase* slam_backend,
  ...
  void addObservation(
      const StereoObservation& st, const id_t observing_kf, const id_t camera_params);
  ...
  const obs_tuple_list_t& allObservations() const { return all_observations_; }
