...
MAP_DEFINITION_REGISTER(
    "mola::SparseTreesPointCloud,SparseTreesPointCloud", mola::SparseTreesPointCloud)
...
void SparseTreesPointCloud::TMapDefinition::dumpToTextStream_map_specific(std::ostream& out) const
{
...
mrpt::maps::CMetricMap::Ptr SparseTreesPointCloud::internal_CreateFromMapDefinition(
...
void    SparseTreesPointCloud::serializeTo(mrpt::serialization::CArchive& out) const
{
  // params:
...
void SparseTreesPointCloud::serializeFrom(mrpt::serialization::CArchive& in, uint8_t version)
{
  switch (version)
  {
    case 0:
...
// Ctor:
SparseTreesPointCloud::SparseTreesPointCloud(float grid_size) { setGridProperties(grid_size); }
...
void SparseTreesPointCloud::getVisualizationInto(mrpt::opengl::CSetOfObjects& outObj) const
{
  ...
  {
    auto lambdaForEachGrid = [this, &outObj](const outer_index3d_t& idxs, const GridData&)
    {
  ...
  if (robotPose)
  {
    ...
    if (insertionOptions.remove_submaps_farther_than > 0)
    {
      eraseGridsFartherThan(
          robotPose3D.translation().cast<float>(), insertionOptions.remove_submaps_farther_than);
    }
  }
  else
  {
    ...
    std::vector<uint64_t>& resultIndicesOrIDs, size_t maxPoints) const
{
  ...
  auto lambdaCheckCell = [&](const outer_index3d_t& p)
  {
    ...
    g->points().nn_radius_search(
        query, search_radius_sqr, sub_results, sub_out_dists_sqr, sub_resultIndicesOrIDs,
        maxPoints);
...
void SparseTreesPointCloud::TInsertionOptions::readFromStream(mrpt::serialization::CArchive& in)
{
  int8_t version;
  in >> version;
  switch (version)
...
void SparseTreesPointCloud::TLikelihoodOptions::readFromStream(mrpt::serialization::CArchive& in)
{
  int8_t version;
  in >> version;
  switch (version)
...
void SparseTreesPointCloud::TInsertionOptions::loadFromConfigFile(
    [[maybe_unused]] const mrpt::config::CConfigFileBase& c, [[maybe_unused]] const std::string& s)
{
    ...
    const std::size_t num_pts)
{
  ...
  uint8_t* doInsert = reinterpret_cast<uint8_t*>(::alloca(sizeof(uint8_t) * num_pts));

  const float minSqrDist = mrpt::square(insertionOptions.minimum_points_clearance);
