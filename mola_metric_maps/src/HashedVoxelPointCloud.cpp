...
MAP_DEFINITION_REGISTER(
    "mola::HashedVoxelPointCloud,HashedVoxelPointCloud", mola::HashedVoxelPointCloud)
...
mrpt::maps::CMetricMap::Ptr HashedVoxelPointCloud::internal_CreateFromMapDefinition(
...
void    HashedVoxelPointCloud::serializeTo(mrpt::serialization::CArchive& out) const
{
  // params:
  out << voxel_size_;
...
void HashedVoxelPointCloud::serializeFrom(mrpt::serialization::CArchive& in, uint8_t version)
{
  switch (version)
  {
    case 0:
...
// Ctor:
HashedVoxelPointCloud::HashedVoxelPointCloud(float voxel_size) { setVoxelProperties(voxel_size); }
...
void HashedVoxelPointCloud::getVisualizationInto(mrpt::opengl::CSetOfObjects& outObj) const
{
  ...
  else
  {
    ...
    const auto lambdaVisitPoints = [&obj, &hists, &nPoints](const mrpt::math::TPoint3Df& pt)
    {
      // x y z R G B [A]
...
void HashedVoxelPointCloud::TInsertionOptions::readFromStream(mrpt::serialization::CArchive& in)
{
  int8_t version;
  in >> version;
  switch (version)
...
void HashedVoxelPointCloud::TLikelihoodOptions::readFromStream(mrpt::serialization::CArchive& in)
{
  int8_t version;
  in >> version;
  switch (version)
...
void HashedVoxelPointCloud::TInsertionOptions::dumpToTextStream(std::ostream& out) const
{
...
void HashedVoxelPointCloud::TLikelihoodOptions::dumpToTextStream(std::ostream& out) const
{
...
void HashedVoxelPointCloud::TRenderOptions::dumpToTextStream(std::ostream& out) const
{
...
const mrpt::maps::CSimplePointsMap* HashedVoxelPointCloud::getAsSimplePointsMap() const
{
