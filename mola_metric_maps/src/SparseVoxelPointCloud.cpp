...
MAP_DEFINITION_REGISTER(
    "mola::SparseVoxelPointCloud,SparseVoxelPointCloud", mola::SparseVoxelPointCloud)
...
void SparseVoxelPointCloud::TMapDefinition::dumpToTextStream_map_specific(std::ostream& out) const
{
...
mrpt::maps::CMetricMap::Ptr SparseVoxelPointCloud::internal_CreateFromMapDefinition(
...
void    SparseVoxelPointCloud::serializeTo(mrpt::serialization::CArchive& out) const
{
  // params:
  ...
  for (const auto& kv : grids_)
  {
    ...
    for (size_t i = 0; i < kv.second.gridData.TOTAL_CELL_COUNT; i++)
    {
      ...
      for (size_t j = 0; j < N; j++) out.WriteAs<uint32_t>(cells[i].getIndex(j));
    }
  }
}
void SparseVoxelPointCloud::serializeFrom(mrpt::serialization::CArchive& in, uint8_t version)
{
  switch (version)
  {
    case 0:
    {
      ...
      for (uint32_t i = 0; i < nGrids; i++)
      {
        ...
        for (size_t k = 0; k < grid.gridData.TOTAL_CELL_COUNT; k++)
        {
          ...
          for (size_t j = 0; j < nPts; j++) cells[k].setIndex(j, in.ReadAs<uint32_t>());
        }
      }
    }
    break;
...
// Ctor:
SparseVoxelPointCloud::SparseVoxelPointCloud(float voxel_size) { setVoxelProperties(voxel_size); }
...
void SparseVoxelPointCloud::setVoxelProperties(float voxel_size)
{
  ...
  gridSizeMinusHalf_ =
      mrpt::math::TPoint3Df(1.0f, 1.0f, 1.0f) * (voxel_size_ * INNER_GRID_SIDE) - halfVoxel_;
...
void SparseVoxelPointCloud::getVisualizationInto(mrpt::opengl::CSetOfObjects& outObj) const
{
  ...
  else
  {
    ...
    {
      const auto lambdaVisitPoints = [&obj, &hists](const mrpt::math::TPoint3Df& pt)
      {
        // x y z R G B [A]
  ...
  {
    auto lambdaForEachGrid = [this, &outObj](const outer_index3d_t& idxs, const InnerGrid&)
    {
...
void SparseVoxelPointCloud::nn_multiple_search(
    [[maybe_unused]] const mrpt::math::TPoint3Df& query, [[maybe_unused]] const size_t N,
    ...
    std::vector<uint64_t>& resultIndicesOrIDs, size_t maxPoints) const
{
  ...
  {
    if (auto [v, grid] = voxelByGlobalIdxs(p, false); v && !v->points(*grid).empty())
    {
...
mrpt::math::TBoundingBoxf SparseVoxelPointCloud::boundingBox() const
{
  if (!cached_.boundingBox_)
  {
    ...
    else
    {
      ...
      auto lambdaForEachGrid = [this](const outer_index3d_t& idxs, const InnerGrid&)
      {
        const mrpt::math::TPoint3Df voxelCenter = globalIdxToCoord(idxs);

        cached_.boundingBox_->updateWithPoint(voxelCenter - halfVoxel_);
        cached_.boundingBox_->updateWithPoint(voxelCenter + gridSizeMinusHalf_);
      };
...
void SparseVoxelPointCloud::visitAllGrids(
    const std::function<void(const outer_index3d_t&, const InnerGrid&)>& f) const
{
  for (const auto& kv : grids_)
  {
    const outer_index3d_t outer_idx = kv.first;
...
void SparseVoxelPointCloud::TInsertionOptions::readFromStream(mrpt::serialization::CArchive& in)
{
  int8_t version;
  in >> version;
  switch (version)
...
void SparseVoxelPointCloud::TLikelihoodOptions::readFromStream(mrpt::serialization::CArchive& in)
{
  int8_t version;
  in >> version;
  switch (version)
...
void SparseVoxelPointCloud::TRenderOptions::writeToStream(mrpt::serialization::CArchive& out) const
{
  const int8_t version = 1;
  out << version;
  out << point_size << show_mean_only << show_inner_grid_boxes << color << int8_t(colormap)
      << recolorByPointField;  // v1
}

void SparseVoxelPointCloud::TRenderOptions::readFromStream(mrpt::serialization::CArchive& in)
{
  int8_t version;
  in >> version;
  switch (version)
...
void SparseVoxelPointCloud::TInsertionOptions::dumpToTextStream(std::ostream& out) const
{
...
void SparseVoxelPointCloud::TLikelihoodOptions::dumpToTextStream(std::ostream& out) const
{
...
void SparseVoxelPointCloud::TRenderOptions::dumpToTextStream(std::ostream& out) const
{
...
auto SparseVoxelPointCloud::VoxelData::points(const InnerGrid& parent) const -> PointSpan
{
